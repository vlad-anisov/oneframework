 ...
 *
 * Половина, ставящая подпись, живёт в ядре (`oneframework keygen` в
 * `libs/js/bin/oneframework.mjs`, ключ читает `build/plan.mjs`), а не в
 * привязке: подписывается сборка, а сборку запускает ядро -- живи команда у
 * привязки, приложение на другом языке подписать было бы нечем.
 *
