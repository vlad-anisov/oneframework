 ...
 *
 * Компиляции здесь больше нет. Расходились они очень тихо: не исключением, а
 * другим WHERE. Самый дорогой случай уже был -- незаданный `view.tag` обязан
 * снимать условие, и SQL это делал, а фронтенд сравнивал напрямую и прятал все
 * строки. Никакой ошибки при этом не видно, видно пустой список.
 *
...
// Компилятор домена -- местный, без WASM. Правила его и охват сторожит
// tests/js/domain.test.mjs: корпус там -- восемь сотен запросов, настоящих и
// краевых.
