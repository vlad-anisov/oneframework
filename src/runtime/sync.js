/**
 * Обмен изменениями между устройствами и сервером.
 *
...
export class SessionApi {
   ...
   *
   * Второго прохода -- пересборки очищенного блоба -- здесь нет, и это
   * разница, а не недоделка: см. `libs/js/src/server.mjs`.
   */
  tablesOf(blob) {
    const { capi, wasm } = this;
    const ptr = wasm.allocFromTypedArray(blob);
 ...
 * нему снимается снимок и по нему же считается отпечаток схемы.
 */
 ...
 * таблицы и упорядоченный список `(колонка, тип SQL)`.
 *
...
/**
 * Сверить отпечатки схем и отказать, если они не сошлись.
 *
 ...
 *
 * Ключ из конверта доказывает только то, что подписавший умеет подписывать. Он
 * сверяется со своим и служит диагностикой: «подписано не тем» понятнее, чем
 * «подпись не сходится».
 *
...
/**
 * Сверить отметки changeset'ов конверта.
 *
...
//: Круга обмена одним вызовом здесь нет нарочно: круг делает воркер
//: (`worker.js`) в два шага, потому что между «отдать» и «наложить» стоит
//: настоящая сеть, а её ответ надо проверить, прежде чем накладывать.
