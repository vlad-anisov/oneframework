 ...
 * Было парой к `oneframework/runtime/session.py` (удалён 21.08.2026).
 *
 ...
 *
 * Было парой к `RowPlan` в `oneframework/runtime/session.py, удалён`. Держать двое врозь -- весь
 * смысл: описание -- факт о **виде**, и едет один раз; гнёзда -- факты о
 * записях, и едут числами. Какое гнездо чьё, написано в самом описании --
 * `"bind": {"value": 3}` на том узле, которому оно нужно, -- поэтому ничего не
 * приходится сопоставлять ни по позиции, ни по договорённости.
 */
 ...
 *
 * Было парой к `_sql_boolean` в `oneframework/runtime/session.py, удалён`; довод сохранён:
 * выбери рантаймы разные дороги -- ячейки разошлись бы на пустом значении и
 * молча. Причина отказа настоящая: `evaluate` отвечает булевым всегда, а SQL на
 * сравнение с пустым краем отвечает **неизвестностью**, и
 * `CASE WHEN NULL THEN 1 ELSE 0 END` делает из неё ноль. Проверено на живой
 * базе: у строки с пустым `a` выражение `a != 1` даёт NULL, колонка даёт 0
 * (=ложь), а `evaluate` на том же месте отвечает true.
 *
...
export class Runtime {
  ...
  snapshot() {
    ...
    return {
      type: "render",
      // стек активного раздела -- это stacks[active]; см. session.py
   ...
   *
   * Было парой к `Runtime.row_plan` в `oneframework/runtime/session.py, удалён`; обязана давать
   * побайтно то же самое: список нёс по целому дереву на запись, и дерево это
   * было каждый раз одним и тем же -- те же номера, те же подписи, те же
   * виджеты, тот же перечень значений той же связи. Различались считанные
   * ответы. Поэтому описание едет один раз, на списке, а запись -- это вектор
   * этих ответов.
   */
