 ...
 * трогала» от «трогала дважды и вернула как было».
 */
...
//: Через сколько шагов вычислителя звать сторожа.
...
//:
//: Выбрана замером: на миллионе строк самый тяжёлый законный запрос --
//: группировка, 12 миллионов шагов. Взято двести, то есть шестнадцатикратный
//: запас, и рез меньше чем за две секунды.
export const STEP_LIMIT = 200_000_000;

/**
 * Сколько раз позвать сторожа, чтобы получился названный предел **шагов**.
 *
 * Отдельной работой ради проверки: спутай здесь шаги с вызовами -- и настоящий
 * предел разойдётся с названным в `STEP_TICK` раз, а увидеть это можно только
 * дождавшись реза. Арифметику можно спросить прямо.
 */
 ...
 *
 * `ALTER TABLE ADD COLUMN` кладёт колонку только в хвост, и этого хватает,
 * чтобы обновившееся устройство и поставленное заново разошлись физически при
 * одном и том же объявлении. changeset адресует значения **номером** колонки:
 * разойдясь, они кладут заголовок в отметку часов, отметку -- в дату создания,
 * а дату -- в версии колонок. Ни исключения, ни спора, отчёт `applied: true`.
 *
 * Надо ли перекладывать, решает сверка **всего** `CREATE TABLE` с тем, что
 * запомнила сама SQLite. Не списка имён: отпечаток схемы считается по
 * `PRAGMA table_info`, то есть тип колонки входит в него наравне с местом, а
 * `ALTER` тип не меняет вовсе. Сверяй мы одни имена -- поле, сменившее тип,
 * осталось бы прежним, и обновившееся устройство разошлось бы отпечатком с
 * поставленным заново навсегда: вечный отказ обмена вместо тихой порчи.
 *
 * Здесь же таблица и создаётся, когда её нет: напиши `CREATE TABLE` ещё и
 * вызывающий, сверка ловила бы расхождение двух наших строк между собой.
 *
 * Данные переносятся по совпадающим именам. Колонка, которой в объявлении
 * больше нет, исчезает вместе с данными, и это осознанная цена: оставь мы её в
 * хвосте «на всякий случай» -- раскладка снова зависела бы от истории
 * устройства, а не от объявления, и обмен снова портил бы данные молча.
 *
 * `legacy_alter_table` здесь свой: переименование иначе переписало бы ссылки
 * на эту таблицу в чужих объявлениях.
 */
...
export class Database {
  constructor(handle, { requestFlush = null, flushNow = null, sqlite3 = null,
    ...
    //: Ставится снаружи (`logic.js`, `validator`); `null` -- правил нет. База
    //: не знает ни про WASM, ни про объявления действий, и знать не должна:
    //: она знает только то, что перед записью кого-то спрашивают.
    this.validator = null;
    ...
    //
    // Признак -- ради сервера на node. Там база живёт **в памяти** --
    // поднимается из файла байтами и выгружается обратно, потому
