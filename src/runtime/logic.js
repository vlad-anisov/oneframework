/**
 * Бизнес-логика в WASM: хост на стороне браузера.
 *
 ...
 * единой сетевой загрузки и без единой зависимости. Здесь три части:
 *
 * * `Api` -- десять вызовов, которые логика имеет право сделать. Ни SQLite, ни
 *   экрана модуль не касается;
 * * `LogicModule` -- разметка кадра и работа с чужой памятью;
 * * чтение модулей из базы.
 *
 * Договор целиком записан в `protocol/logic.json`.
 *
...
/** Версия договора: содержимое кадра. Не то же, что ABI. */
...
/** Значение из базы -> значение на проводе. */
...
// исполнение модуля
// --------------------------------------------------------------------------
// Разворота модуля WASM здесь нет: действия описываются данными, и
// разворачивать нечего. См. `src/rel/action.js`.
...
//: Приёма **байтов** модуля обменом здесь нет: действия едут объявлениями,
//: той же дорогой, что модели и виды.
 ...
 * именно `ids` значило бы протащить соглашение фреймворка внутрь чужого
 * языка.
 */
