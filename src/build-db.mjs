 ...
 * из приложения: раньше план строила питоновская копия
 * (`oneframework/cli/plan.py`, удалена), и правило жило в двух местах.
 *
...
const ответ = {};
try {
  // Файл может уже существовать -- `inspect --db` выкладывает приложение
  // поверх базы, которая у пользователя уже есть, и определения в ней надо
  // обновить, а не потерять вместе с данными.
  ...
  // (`inspect --db` открывает ту, что уже есть у пользователя), и второй посев
  // положил бы те же записи ещё раз -- то есть упал бы на ключе, а до правки
  // 21.08.2026 именно так и падал.
  //
  // Ключи записей пришли готовыми: их выдал питон, потому что посев ими
  // пользуется на месте (`tag = db.create(...)`), и они обязаны быть
  // одинаковыми на всех клиентах.
  let строк = 0;
