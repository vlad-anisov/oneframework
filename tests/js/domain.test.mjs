 ...
 * была эталоном для сверки; 21.08.2026 держателем стала та, которую правда
 ...
 *
 * Переписана с питона 21.08.2026: он был здесь только оснасткой -- гонял
 * долгоживущий хост построчным протоколом. Правила от этого не менялись, а
...
describe("домен: правила", () => {
  ...
  it("узел «пусто» превращается в IS NULL и только на колонке", () => {
    // Отдельно от `= null`: это разные узлы, и до 21.08.2026 прямой проверки
