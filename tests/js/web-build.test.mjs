 ...
 *
 * Пока сборщик переезжал, здесь стояла двусторонняя сверка с
 * `oneframework/cli/builders/web.py`. Она нашла то, ради чего и ставилась:
 * питон сортирует пути **по частям**, а не по строке, и обход по строкам
 * поставил бы «a/b.js» раньше «a/b/c/deep.js». Питоновской половины больше нет;
 * правило осталось, и записано здесь прямо.
 *
