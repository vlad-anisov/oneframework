 ...
 *
 * Базу приложения пишет одна сторона -- `src/build-db.mjs`, -- и
 * правила здесь не про совпадение копий, а про поведение:
 *
...
test("один-ко-многим читается обратной колонкой потомка", () => {
  // Дорога своя, и сторожить её надо отдельно: сломанный `readOne2many`
  // оставляет всю сюиту зелёной -- ни одна проверка не заметит, что список
  // потомков стал пустым.
...
test("удаление записи обнуляет ссылки на неё", () => {
  ...
  // пустоту вместо имени, а причину видно только в базе. Прежде правило
