 ...
 * синхронным.
 *
 * Здесь эта дорога и осталась единственной.
 */
 ...
 *
 * Отдельно от `boot`, потому что зовётся не только на запуске. Схема и
 * документы читаются из базы, а не из сборки, -- и обещание, что устройство,
 * получившее вид обменом, начинает рисовать его без переустановки, держится
 * только если после обмена перечитать. Читай мы **один раз за жизнь
 * воркера**, приехавший вид лежал бы в `_oneframework_def` до следующего
 * запуска.
 *
 * Дорого это ровно настолько, насколько дорог запуск, -- и платится только
 * тогда, когда определения правда менялись.
 */
