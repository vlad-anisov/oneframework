 ...
 *
 * Уровень называется ключом кадра, а не номером: между снимком, по которому
 * нарисованы крошки, и нажатием лежит круг через воркер (`back_to`, разобрано
 * в `oneframework/runtime/session.py`, удалён).
 *
            ...
            <React.Fragment key={frame.id}>
              ...
              <BreadcrumbsItem active={here} data-index={at}>
                {/* `name`, а не `title`: пустой заголовок -- это отказ бара
                    ...
                    звено без слова было бы дырой в цепочке. Разобрано у
                    `Frame.name`, прежде в `oneframework/runtime/session.py`. */}
                {here ? frame.name : (
                  <a
                    href="#"
                    onClick={(e) => {
